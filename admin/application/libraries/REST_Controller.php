...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _prepare_digest_auth() {
        ...
        $digest['nonce'] = (isset($digest['nonce'])) ? $digest['nonce'] : '';
        $digest['nc'] = (isset($digest['nc'])) ? $digest['nc'] : '';
        $digest['cnonce'] = (isset($digest['cnonce'])) ? $digest['cnonce'] : '';
    ...
    public function check_token($check_token_only = false) {
        $this->load->model('Auth_model', 'auth');
        
        if (!$this->auth->check_token($this->input->server('HTTP_TOKEN'))) {
            $data = array(
                'success' => FALSE,
                'msg' => 'Invalid token',
        ...
        if($check_token_only){
           ...
           $user_details = $this->user->get_detail($user_id);
           if($arm == $user_details['arm_alloted']){
                $this->response(array('success' => true, 'msg' => 'Valid Token', 'data' => $user_details), REST_Controller::HTTP_OK);
           }else{
               $data = array(
                'success' => FALSE,
                'msg' => 'Invalid token',
    ...
    public function get_user($token = NULL) {
        $this->load->model('Auth_model', 'auth');
        $user_id = $this->auth->get_user($this->input->server('HTTP_TOKEN'));
        if ($user_id > 0) {
            return $user_id;
        } else{
            $data = array(
